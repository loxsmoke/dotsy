...
- **Stale-build heuristic** watches build/test commands for the poisoned-`obj/` signature: several consecutive failed builds whose error message *keeps changing* even though each previous error was fixed (incremental builds on stale intermediate state produce rotating phantom errors a model never diagnoses on its own). After three consecutive failures spanning at least two distinct error signatures, a hint telling the model to clean-rebuild first is injected after that turn's tool results. A passing build resets the tracking and re-arms the hint.
- **Per-call safeguards** run inside tool execution: an `Edit`/`Write`/`MultiEdit` of a file the model has not `Read` this session — or whose on-disk state changed since the last read — is rejected before the approval prompt (`agent.require_read_before_edit`), telling the model to re-read instead of editing blind; a repeated `Read` of a file whose content is still live in context returns a short stub instead of the full content (`agent.dedupe_reads`); and time spent waiting on the user's approval prompt is measured separately from the tool's run time, so `ToolFinished.DurationMs` and the session log's `duration_ms` reflect pure execution.
...
public record TokenUsageUpdated(int InputTokens, int OutputTokens,
    ...
    long DurationMs = 0, long? ServerDurationMs = null)      : LoopEvent; // exposes TokensPerSecond (server-measured preferred)
public record CompactionOccurred(int TokensBefore, int TokensAfter, string Summary) : LoopEvent;
public record CompactionSkipped(string Reason)               : LoopEvent; // compaction ran but freed nothing
...
When the model returns multiple tool calls in one response and `agent.parallel_tools` is enabled, tools declared `ToolSafety.ReadOnly` (`Read`, `Glob`, `Grep`, `List`, …) are executed concurrently via `Task.WhenAll`. All other tools (`Write`, `Edit`, `Shell`, …) are executed sequentially in call order to avoid conflicts. Each tool declares `ToolSafety { ReadOnly | Sequential | Destructive }`.
