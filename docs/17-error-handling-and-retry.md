...
- **Max retries:** 10 (**pi** pattern, configurable via `DOTSY_MAX_RETRIES` env var)

The TUI status bar shows "Retrying in Ns · attempt x/10" during waits.
...
public record RequestError(int StatusCode)           : ProviderError;  // other 4xx — not retried
