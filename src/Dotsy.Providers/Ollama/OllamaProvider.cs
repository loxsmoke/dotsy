        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (done)
            {
                ...
                if (chunk.TryGetProperty("eval_count", out var evalCount))
                {
                    ...
                    // eval_duration is generation time in nanoseconds, measured server-side.
                    long? evalMs = chunk.TryGetProperty("eval_duration", out var ed)
                        && ed.TryGetInt64(out var ns) && ns > 0
                        ? ns / 1_000_000
                        : null;
                    yield return new UsageUpdate(promptTokens, evalCount.GetInt32(), 0, 0, evalMs);
                }
