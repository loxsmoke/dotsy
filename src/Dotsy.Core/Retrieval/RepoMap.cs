...
public static class RepoMap
{
    ...
    private static bool IsGeneratedFile(string fileName, string outline) =>
        fileName.EndsWithNoCase(".g.cs")
        || fileName.EndsWithNoCase(".designer.cs")
        || fileName.EndsWithNoCase(".generated.cs")
        || outline.ContainsNoCase("<auto-generated");

    private static bool IsPropertyHeavy(string outline)
    {
        var memberLines = outline.Split('\n')
            .Select(l => l.Trim())
            .Where(l => l.StartsWithNoCase("public ")
                || l.StartsWithNoCase("internal ")
                || l.StartsWithNoCase("protected ")
                || l.StartsWithNoCase("private "))
            .ToList();
