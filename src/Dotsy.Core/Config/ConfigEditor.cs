...
public static class ConfigEditor
{
    ...
    [
        ...
        [
            ...
            Param("nudge_limit", "int", "max consecutive non-terminal text-only turns before stopping (headless)",
                cfg => cfg.Agent.NudgeLimit),
            Param("interactive_nudge_limit", "int", "nudge limit used by the interactive TUI (1 = yield after one stalled turn)",
                cfg => cfg.Agent.InteractiveNudgeLimit),
            Param("auto_continue_on_nudge", "bool", "inject a recovery hint and retry on a recoverable nudge instead of stopping",
                cfg => cfg.Agent.AutoContinueOnNudge),
            Param("auto_continue_max_attempts", "int", "max progress-guarded auto-continue retries before giving up",
                cfg => cfg.Agent.AutoContinueMaxAttempts),
            Param("auto_continue_on_end_turn_intent", "bool", "retry when a text-only turn announces the next step but calls no tool, instead of stopping",
                cfg => cfg.Agent.AutoContinueOnEndTurnIntent),
